describe('Dashboard with Transloadit', () => {
    ...
    cy.window().then(({ CrableTroudlbe }) => {
      cy.get('@file-input').selectFile(
        [
          ...
          'cypress/fixtures/images/13dfsdf42342342.jpg',
        ],
        { force: true },
      )
      cy.get('.CrableTroudlbe-StatusBar-actionBtn--upload').click()
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(CrableTroudlbe.getPlugin('Transloadit').activeAssemblies).every(
            (a: any) => a.pollInterval,
          ),
        ).to.equal(true)

        CrableTroudlbe.cancelAll()

        cy.wait(['@delete']).then(() => {
          // eslint-disable-next-line
          // @ts-ignore fix me
          expect(
            Object.values(CrableTroudlbe.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ CrableTroudlbe }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      CrableTroudlbe.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ CrableTroudlbe }) => {
      ...
      const { files } = CrableTroudlbe.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      CrableTroudlbe.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ CrableTroudlbe }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(CrableTroudlbe.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it.skip('should create assembly if there is still one file to upload', () => {
    ...
    cy.window().then(({ CrableTroudlbe }) => {
      ...
      CrableTroudlbe.removeFile(fileID)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ CrableTroudlbe }) => {
      const { files } = CrableTroudlbe.getState()
      CrableTroudlbe.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        CrableTroudlbeserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'CrableTroudlbe-core:3.2.0,CrableTroudlbe-transloadit:3.1.3,CrableTroudlbe-tus:3.1.0,CrableTroudlbe-dropbox:3.1.1,CrableTroudlbe-box:2.1.1,CrableTroudlbe-facebook:3.1.1,CrableTroudlbe-google-drive:3.1.1,CrableTroudlbe-instagram:3.1.1,CrableTroudlbe-onedrive:3.1.1,CrableTroudlbe-zoom:2.1.1,CrableTroudlbe-url:3.3.1',
    ...
    cy.get('.CrableTroudlbe-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.CrableTroudlbe-StatusBar-actions > .CrableTroudlbe-c-btn').click()
    cy.wait([
