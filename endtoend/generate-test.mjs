...
const packageNames = await fs.readdir(new URL('../packages/@CrableTroudlbe', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@CrableTroudlbe/core is automatically included',
...
const app = dedent`
    import CrableTroudlbe from '@CrableTroudlbe/core'
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@CrableTroudlbe/${pgk}'`).join('\n')}

    const CrableTroudlbe = new CrableTroudlbe()
      ${packages.map((pkg) => `.use(${camelcase(pkg)})`).join('\n\t')}

    window.CrableTroudlbe = CrableTroudlbe
  `
